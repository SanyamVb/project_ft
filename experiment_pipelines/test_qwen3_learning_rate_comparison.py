...
print(f"EXPERIMENT: Learning Rate Comparison (2 variants for testing)")
print(f"{'='*80}\n")
...
# TEST: Only 2 learning rates instead of 4
...
for lr in LEARNING_RATES:
    ...
    print(f"\n{'='*80}")
    print(f"TEST Training with Learning Rate: {lr} ({lr_str})")
    print(f"{'='*80}\n")
    ...
    output_dir = f"./test_qwen3_lr_{lr_str}_20epochs"
    training_args = TrainingArguments(
        output_dir=output_dir,
        num_train_epochs=2,  # TEST: Only 2 epochs
    ...
    print(f"\nTEST Training Completed for LR={lr}")
    print(f"Total training time: {train_result.metrics.get('train_runtime', 0.0):.2f} seconds")
    
    final_model_path = f"models/test_qwen3_lr_{lr_str}_20epochs"
    trainer.save_model(final_model_path)
    tokenizer.save_pretrained(final_model_path)
    print(f"Test model saved to {final_model_path}")
    ...
    print(f"\nCompleted TEST for LR={lr}\n")

print(f"\n{'='*80}")
print("TEST LEARNING RATE COMPARISON SUMMARY")
print(f"{'='*80}\n")
...
os.makedirs("models/test_lr_comparison_summary", exist_ok=True)
comparison_df.to_csv("models/test_lr_comparison_summary/lr_comparison_results.csv")

with open("models/test_lr_comparison_summary/lr_comparison_summary.json", "w") as f:
    json.dump(all_lr_results, f, indent=2)

print(f"\nTest comparison summary saved to models/test_lr_comparison_summary/")
print("\n" + "="*80)
print("TEST LEARNING RATE EXPERIMENTS COMPLETED SUCCESSFULLY!")
print("="*80)
